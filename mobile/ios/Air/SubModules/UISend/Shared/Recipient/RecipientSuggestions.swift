        ...
        let suggestions = model.$account.savedAddresses.getMatching(searchString)
            .map { savedAddress in
                SavedAddressSuggestion(
                    ...
                    isCompatibleWithActiveChain: savedAddress.chain == model.chain
                )
            }

        switch model.recipientPolicy.suggestionStrategy {
        case .all:
            return suggestions
        case .requireActiveChain:
            return suggestions.filter(\.isCompatibleWithActiveChain)
    ...
    private var matchingAccounts: [AccountSuggestion] {
        ...
        let suggestions = otherAccountIds
            .compactMap { accountId -> AccountSuggestion? in
                ...
                if model.recipientPolicy.suggestionStrategy == .requireActiveChain, !isCompatible {
                    return nil
                }
                ...
                return AccountSuggestion(
                    ...
                    isCompatibleWithActiveChain: isCompatible
                )
            }

        switch model.recipientPolicy.suggestionStrategy {
        case .all, .requireActiveChain:
            return suggestions
        case .preferActiveChain:
            return suggestions.sortedByCompatibility
        }
...
private struct SavedAddressSuggestion: Hashable {
    ...
    let isCompatibleWithActiveChain: Bool
}
...
private extension Array where Element == SavedAddressSuggestion {
    var sortedByCompatibility: [SavedAddressSuggestion] {
        filter(\.isCompatibleWithActiveChain) + filter { !$0.isCompatibleWithActiveChain }
    }
}
...
private struct SavedAddressButton: View {
    ...
    private var incompatibilityText: String? {
        guard model.recipientPolicy.suggestionStrategy == .preferActiveChain,
    ...
    func onTap() {
        model.selectSavedAccount(
            account.wrappedValue,
            saveKey: savedAddress.address,
            fallbackChain: savedAddress.chain
        )
        endEditing()
    }
...
private struct AccountButton: View {
    ...
    func onTap() {
        model.selectAccount(
            account.wrappedValue,
            fallbackChain: suggestion.selectedChain
        )
        endEditing()
    }
}
